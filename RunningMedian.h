...
public:
  ...
  uint16_t getQuantile(const float quantile);
  ...
  uint16_t getAverage(uint8_t nMedian);

  uint16_t getHighest() { return getSortedElement(_count - 1); };
  uint16_t getLowest() { return getSortedElement(0); };

  // get n-th element from the values in time order
  uint16_t getElement(const uint8_t n);
  // get n-th element from the values in size order
  uint16_t getSortedElement(const uint8_t n);
  // predict the max change of median after n additions
  uint16_t predict(const uint8_t n);
...
protected:
  ...
  uint8_t *_sortIdx;
#else
  uint16_t _values[MEDIAN_MAX_SIZE];
